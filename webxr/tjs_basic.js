...
async function sceneFromProject(url) {
  ...
  if (r.status != 200) {
    log(`Failed to load ${url} : ${r.status}`);
    return null
  }
  const project = await r.json();
  log("Project loaded and parsed");
  log("Init THREE.ObjectLoader");
  const ldr = new THREE.ObjectLoader();
  try {
    const scene = await ldr.parseAsync(project.scene);
    return scene
  } catch (e) {
    log(`Parsing scene from ${url} FAILED : ${e}`);
  }
  return null;
}
...
function ctrlHighInit(xr) {
	...
	c0.addEventListener( 'selectstart', ()=>{userMove(xr, -1)})
	// c0.addEventListener( 'selectend', ()=>{})
	c0.addEventListener( 'squeezestart', ()=>{userMove(xr, -0.1)})
	// c0.addEventListener( 'squeezeend', ()=>{})

	const c1=xr.getController( 1 ) // Right
	c1.addEventListener( 'selectstart', ()=>{userMove(xr, 1)})
	// c1.addEventListener( 'selectend', ()=>{})
	c1.addEventListener( 'squeezestart', ()=>{userMove(xr, 0.1)})
 ...
 * */
function userOffsetTransfo(xr, offsetTransfo) {
  xr.setReferenceSpace(xr.getReferenceSpace().getOffsetReferenceSpace(offsetTransfo));
}

function userTeleportOffsetXZ(xr, delta_x, delta_z) {
	userOffsetTransfo(xr, new XRRigidTransform({ x: -delta_x, y: 0, z: -delta_z, w: 1 }))
}

function userRotate(xr, angleDeg) {
	...
	userOffsetTransfo(xr, transform)
}

function userDelta(xr, dist) {
...
function userSlide(xr, dist) {
	console.log("slide", dist)
	const delta = userDelta(xr, dist)
	// we just exchange z and x
	userTeleportOffsetXZ(xr, delta.z, delta.x)
}

/** walk using the direction of the camera */
function userMove(xr, dist) {
	console.log("move", dist)
	const delta = userDelta(xr, dist)
	userTeleportOffsetXZ(xr, delta.x, delta.z)
}

function hudSet(hud, a, b, c) {
	if(!hud) return
...
//	statsVR.setY(1)
	...
	const my_controllers = {
		...
		right: ctrlInit('right',{
			...
			ax:()=>{userMove(renderer.xr, -0.1)}
		}),
		left: ctrlInit('left',{
			trig:()=>{userSlide(renderer.xr, -0.1)},
			grip:()=>{userSlide(renderer.xr, 0.1)},
			up:()=>{userMove(renderer.xr, 0.1)},
			down:()=>{userMove(renderer.xr, -0.1)},
			left:(c)=>{userSlide(renderer.xr,c.grip.pressed?0.1:1)},
			right:(c)=>{userSlide(renderer.xr,c.grip.pressed?-0.1:-1)}
		})
	}
...
function ctrlCheckPressAction(c, obj, pressed, info) {
	// if(!obj.cb) return
	if(!obj.pressed && pressed) {
		// myLog('Pressed '+info+' '+c.name)
		hudSet(HUD, 'Pressed', info+' '+c.name)
		obj.pressed=true
		if(obj.cb) obj.cb(c)
	}
	if(obj.pressed && !pressed) {
...
function ctrlErr(errObj, where, e) {
	if(!errObj) return
	if(errObj.firstError) return
	try {
		errObj.firstError = `Error in ${where} : ${e}`
		log(errObj.firstError)
		hudSet(HUD, "ERROR", "loopCtrlLow", e)
	}
	catch(_e2){
		1
	}
...
function ctrlEvent(myCtrl, gamepad) {
	// https://developer.mozilla.org/en-US/docs/Web/API/Gamepad
	if(!myCtrl) return
	if(!gamepad) return // possible if XR session not started yet
	if(!gamepad.buttons)  { ctrlErr(myCtrl, 'ctrlEvent '+myCtrl.name, 'No buttons'); return }
	if(!gamepad.buttons.length)	{ ctrlErr(myCtrl, 'ctrlEvent '+myCtrl.name, 'Empty buttons array'); return }

	// if(!c.firstMsg) { myLog(`${c.name} first ctrlEvent ...`) }

  // const _trig = g.buttons[0]
  // const _grip = g.buttons[1]  		
	ctrlCheckPressAction(myCtrl, myCtrl.trig, gamepad.buttons[0]?.pressed, "TRIG")
	ctrlCheckPressAction(myCtrl, myCtrl.grip, gamepad.buttons[1]?.pressed, "GRIP")
	ctrlCheckPressAction(myCtrl, myCtrl.thb, gamepad.buttons[3]?.pressed, "THUMB")
	ctrlCheckPressAction(myCtrl, myCtrl.ax, gamepad.buttons[4]?.pressed, "A/X")
	ctrlCheckPressAction(myCtrl, myCtrl.by, gamepad.buttons[5]?.pressed, "B/Y")

	if(!gamepad.axes) { ctrlErr(myCtrl, 'ctrlEvent '+myCtrl.name, 'No axes'); return }
	if(!gamepad.axes.length) { ctrlErr(myCtrl, 'ctrlEvent '+myCtrl.name, 'Empty axes array'); return }
	if(gamepad.axes.length<2) { ctrlErr(myCtrl, 'ctrlEvent '+myCtrl.name, 'Not at leat two axes, but: '+gamepad.axes.length); return }
	ctrlCheckPressAction(myCtrl, myCtrl.left, gamepad.axes.length>2 && gamepad.axes[2]<-0.95, "THB X Left")
	ctrlCheckPressAction(myCtrl, myCtrl.right, gamepad.axes.length>2 && gamepad.axes[2]>0.95, "THB X Right")
	ctrlCheckPressAction(myCtrl, myCtrl.up, gamepad.axes.length>3 && gamepad.axes[3]<-0.95, "THB Y Up")
	ctrlCheckPressAction(myCtrl, myCtrl.down, gamepad.axes.length>3 && gamepad.axes[3]>0.95, "THB Y Down")
	// if(!c.firstMsg) {
	//  	myLog(`${c.name} first ctrlEvent axes: ${g.hand} ${g.axes.length} ${g.axes[0]} ${g.axes[1]} ${g.axes[2]} ${g.axes[3]}`)
	//  	c.firstMsg = true
	// }
}

function ctrlLoop(xr, myControllers) {
	try {
	  ...
	  const inputSources = session.inputSources;
		...
		// 	myLog(`first ctrlLoop , INPUTS : ${inputSources?inputSources.length:'NONE !'}`)
		// 	my_controllers.firstMsg = true
		// }

	  ctrlEvent(myControllers.left, inputSources[0]?.gamepad)
	  ctrlEvent(myControllers.right, inputSources[1]?.gamepad)
	  return
	}
	catch(e) {
		ctrlErr(myControllers, 'ctrlLoop', e)
	}
}
